    ...
    async def _process_message(
        ...
        if msg_type == "agent_register":
            ...
            c.execute(
                ...
                (id, hostname, ip, os, arch, version, status, registered_at, last_seen,
...
def create_web_app(
    ...
    def list_agents():
        ...
        c = conn.cursor()
        c.execute("""
            SELECT id, hostname, ip, os, arch, version, status,
    ...
    def list_software():
        ...
        query = """SELECT s.id, s.name, s.version, s.vendor, s.install_path as installPath,
                         ...
                         (SELECT COUNT(DISTINCT c.cve_id FROM cve_software cs
                          JOIN cve_records c ON cs.cve_id = c.id
                          WHERE LOWER(cs.software_name) LIKE '%' || LOWER(s.name) || '%') as cveCount
